...
#
# DOES:
#   -trolls the Koha database for biblios containing a 999c, and copies it to the 001 tag
#
# CREATES:
#   -nothing
#
...
if ($new_only) {
   $sth = $dbh->prepare("SELECT biblionumber from biblio where datecreated = CURDATE()");
}
else {
   $sth = $dbh->prepare("SELECT biblionumber from biblio");
}   
